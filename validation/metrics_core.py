...
- SDR    = std(u) / std(x) using pairwise-valid dates only
...
def compute_station_metrics(
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        ...
        std_u = np.nanstd(np.where(valid, u, np.nan), axis=0, ddof=1)
        std_x = np.nanstd(np.where(valid, x, np.nan), axis=0, ddof=1)
