...
"""
01_train_xgb005_monthly.py

Train monthly XGB-005 models with repeated spatial block cross-validation:
- X: 0.25-degree predictors aggregated from 0.05-degree static predictors
- y: 0.25-degree monthly climatology of Tmax

Inputs:
- predictors_025_from005.nc
- tmax_025_clim_monthly.nc

Outputs:
- xgb005_month_01.joblib ... xgb005_month_12.joblib
- xgb005_train_metrics.csv
- xgb005_feature_importance.csv
- tmax_025_clim_xgb005_fit.nc
"""
...
DEFAULT_PRED_PATH = "/public/home/ggao001/users/xhang/Projects/CN_YANHAI_DOWN/01data/coastal_static/predictors_025_static.nc"
DEFAULT_CLIM_PATH = "/public/home/ggao001/users/xhang/Projects/zcn/tmax/04xgb/02exp/xgb005/interim/tmax_025_clim_monthly.nc"

DEFAULT_MODEL_DIR = "/public/home/ggao001/users/xhang/Projects/zcn/tmax/04xgb/02exp/xgb005/models"
DEFAULT_METRICS_CSV = "/public/home/ggao001/users/xhang/Projects/zcn/tmax/04xgb/02exp/xgb005/models/xgb005_train_metrics.csv"
DEFAULT_IMPORTANCE_CSV = "/public/home/ggao001/users/xhang/Projects/zcn/tmax/04xgb/02exp/xgb005/models/xgb005_feature_importance.csv"
DEFAULT_FIT_NC = "/public/home/ggao001/users/xhang/Projects/zcn/tmax/04xgb/02exp/xgb005/interim/tmax_025_clim_xgb005_fit.nc"

TARGET_VAR_NAME = None

DEFAULT_N_ESTIMATORS_GRID = "400,800"
DEFAULT_MAX_DEPTH_GRID = "8,12"
DEFAULT_LEARNING_RATE_GRID = "0.03,0.05"
DEFAULT_SUBSAMPLE_GRID = "0.8"
DEFAULT_COLSAMPLE_BYTREE_GRID = "0.8"
DEFAULT_MIN_CHILD_WEIGHT_GRID = "5,8"
DEFAULT_REG_ALPHA_GRID = "0.0"
DEFAULT_REG_LAMBDA_GRID = "1.0"
...
def load_target(clim_path: str):
    ...
    if TARGET_VAR_NAME is not None:
        ...
        target_var = find_monthly_target_var(
            ...
            lat_name,
            lon_name,
            preferred_names=["clim_tmax", "tmax_clim", "tmax", "Band1"],
        )
...
def parse_float_list(text: str):
    ...
    if not values:
        raise ValueError("Parsed empty float grid.")
    return values
...
def build_param_grid(args):
    param_dict = {
        ...
        "learning_rate": parse_float_list(args.learning_rate_grid),
        "subsample": parse_float_list(args.subsample_grid),
        "colsample_bytree": parse_float_list(args.colsample_bytree_grid),
        "min_child_weight": parse_float_list(args.min_child_weight_grid),
        "reg_alpha": parse_float_list(args.reg_alpha_grid),
        "reg_lambda": parse_float_list(args.reg_lambda_grid),
    }
...
def evaluate_param_grid(
    ...
    cv_rows.sort(
        key=lambda r: (
            ...
            r["max_depth"] if r["max_depth"] is not None else 9999,
            r["n_estimators"],
        )
    )
    return cv_rows
...
def train_final_model(x_valid, y_valid, best_params, random_state, tree_method: str):
    model = XGBRegressor(
...
def main():
    parser = argparse.ArgumentParser(description="Train monthly XGB-005 models with repeated spatial block CV.")
    ...
    parser.add_argument("--model-dir", default=DEFAULT_MODEL_DIR, help="Output directory for monthly models")
    ...
    parser.add_argument("--learning-rate-grid", default=DEFAULT_LEARNING_RATE_GRID, help="Comma-separated learning_rate search grid")
    parser.add_argument("--subsample-grid", default=DEFAULT_SUBSAMPLE_GRID, help="Comma-separated subsample search grid")
    parser.add_argument("--colsample-bytree-grid", default=DEFAULT_COLSAMPLE_BYTREE_GRID, help="Comma-separated colsample_bytree search grid")
    parser.add_argument("--min-child-weight-grid", default=DEFAULT_MIN_CHILD_WEIGHT_GRID, help="Comma-separated min_child_weight search grid")
    parser.add_argument("--reg-alpha-grid", default=DEFAULT_REG_ALPHA_GRID, help="Comma-separated reg_alpha search grid")
    parser.add_argument("--reg-lambda-grid", default=DEFAULT_REG_LAMBDA_GRID, help="Comma-separated reg_lambda search grid")
    parser.add_argument("--tree-method", default=DEFAULT_TREE_METHOD, help="XGBoost tree_method")
    ...
    for im, month_value in enumerate(months):
        ...
        model_path = os.path.join(args.model_dir, f"xgb005_month_{month_int:02d}.joblib")
        payload = {
            "model": final_model,
        ...
        for feat_name, feat_imp in zip(predictor_names, final_model.feature_importances_):
    ...
    ds_fit = xr.Dataset(
        ...
        attrs={
            "title": "XGB-005 fitted monthly climatology on 0.25-degree grid",
            ...
            "search_grid": json.dumps({
                ...
                "learning_rate_grid": parse_float_list(args.learning_rate_grid),
                "subsample_grid": parse_float_list(args.subsample_grid),
                "colsample_bytree_grid": parse_float_list(args.colsample_bytree_grid),
                "min_child_weight_grid": parse_float_list(args.min_child_weight_grid),
                "reg_alpha_grid": parse_float_list(args.reg_alpha_grid),
                "reg_lambda_grid": parse_float_list(args.reg_lambda_grid),
            }, ensure_ascii=False),
        },
    )

    ds_fit["clim_tmax_xgb005_fit"].attrs = {
        "long_name": "XGB-005 fitted monthly climatology on training grid",
    ...
    ds_fit.to_netcdf(
        args.fit_nc,
        encoding={"clim_tmax_xgb005_fit": {"zlib": True, "complevel": 4, "dtype": "float32"}},
    )
