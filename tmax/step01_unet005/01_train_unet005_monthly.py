...
"""
01_train_unet005_monthly.py

Train monthly UNet-005 models:
...
Outputs:
- unet005_month_01.pt ... unet005_month_12.pt
- unet005_train_metrics.csv
- unet005_feature_importance_proxy.csv
- tmax_025_clim_unet005_fit.nc
"""
...
# Default paths
# =========================================================
DEFAULT_PRED025_PATH = "/public/home/ggao001/users/xhang/Projects/CN_YANHAI_DOWN/01data/coastal_static/predictors_025_static.nc"
DEFAULT_CLIM025_PATH = "/public/home/ggao001/users/xhang/Projects/zcn/tmax/05unet/02exp/unet005/interim/tmax_025_clim_monthly.nc"

DEFAULT_MODEL_DIR = "/public/home/ggao001/users/xhang/Projects/zcn/tmax/05unet/02exp/unet005/models"
DEFAULT_METRICS_CSV = "/public/home/ggao001/users/xhang/Projects/zcn/tmax/05unet/02exp/unet005/models/unet005_train_metrics.csv"
DEFAULT_IMPORTANCE_CSV = "/public/home/ggao001/users/xhang/Projects/zcn/tmax/05unet/02exp/unet005/models/unet005_feature_importance_proxy.csv"
DEFAULT_FIT_NC = "/public/home/ggao001/users/xhang/Projects/zcn/tmax/05unet/02exp/unet005/interim/tmax_025_clim_unet005_fit.nc"
...
def main():
    parser = argparse.ArgumentParser(description="Train monthly UNet-005 models.")
    parser.add_argument("--pred025", default=DEFAULT_PRED025_PATH)
    parser.add_argument("--clim025", default=DEFAULT_CLIM025_PATH)
    ...
    x_maps, predictor_names, feature_means, feature_stds, pred_mask, pred_lat, pred_lon = load_clim_predictor_maps(
        args.pred025,
        exclude_vars="",  # no feature exclusion (include lat2d/lon2d)
        clim_start_year=args.clim_start_year,
        clim_end_year=args.clim_end_year,
    )
    ds_tgt, month_name, tgt_lat_name, tgt_lon_name, target_var = load_target(args.clim025)
    ...
    for im, month_value in enumerate(months):
        ...
        model_path = os.path.join(args.model_dir, f"unet005_month_{month_int:02d}.pt")
        payload = {
            "month": month_int,
            "feature_names": predictor_names,
            ...
            "predictor_file": args.pred025,
            "target_file": args.clim025,
    ...
    if args.month is not None:
        ...
        ds_fit_part = xr.Dataset(
            {
                "clim_tmax_unet005_fit": (("month", "lat", "lon"), fit_cube[month_row:month_row + 1].astype(np.float32)),
            },
            coords={
        ...
        ds_fit_part.to_netcdf(
            fit_part_path,
            encoding={"clim_tmax_unet005_fit": {"zlib": True, "complevel": 4, "dtype": "float32"}},
        )
        ds_tgt.close()
    ...
    ds_fit = xr.Dataset(
        ...
        attrs={
            "title": "UNet-005 fitted monthly climatology on 0.25-degree grid",
    ...
    ds_fit["clim_tmax_unet005_fit"].attrs = {
        "long_name": "UNet-005 fitted monthly climatology on training grid",
