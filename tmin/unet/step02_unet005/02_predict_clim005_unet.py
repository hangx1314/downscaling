...
"""
07_predict_clim005_unet.py

Predict 0.05-degree monthly climatology using trained UNet-005 monthly models.
...
# Default paths
# =========================================================
DEFAULT_PRED005_PATH = "./data/coastal_static/predictors_005_static.nc"
...
DEFAULT_OUT_PATH = "./outputs/tmin/unet/unet005/interim/tmin_005_clim_unet.nc"
...
def get_model_path(model_dir: str, month_int: int) -> str:
    return os.path.join(model_dir, f"unet005_month_{month_int:02d}.pt")
...
def main():
    parser = argparse.ArgumentParser(description="Predict 0.05-degree monthly climatology using trained UNet-005 models.")
    ...
    first_model_path = find_first_model_path(args.model_dir, "unet005", train_months)
    ...
    for im, month_int in enumerate(month_values):
        ...
        model = U_Net(in_ch=x_np.shape[0], out_ch=1, base_channels=int(payload.get("base_channels", 64))).to(device)
    ...
    ds_out = xr.Dataset(
        data_vars={
            "clim_tmin_unet": (("month", "lat", "lon"), clim_out.astype(np.float32))
        },
        coords={
        ...
        attrs={
            "title": "UNet-005 predicted monthly climatology on 0.05-degree grid",
    ...
    ds_out["clim_tmin_unet"].attrs = {
        "long_name": "UNet-005 predicted monthly climatology of Tmin on 0.05-degree grid",
        "units": "",
    }
    ...
    encoding = {"clim_tmin_unet": {"zlib": True, "complevel": 4, "dtype": "float32"}}
