...
"""
16_predict_clim001_xgb.py

Predict 0.01-degree monthly climatology using trained XGB-001 monthly models.
...
DEFAULT_PRED_PATH = "/public/home/ggao001/users/xhang/Projects/CN_YANHAI_DOWN/01data/coastal_static/predictors_001_static.nc"
DEFAULT_MODEL_DIR = "/public/home/ggao001/users/xhang/Projects/zcn/tmin/04xgb/02exp/xgb001/models"
DEFAULT_SOURCE_CLIM = "/public/home/ggao001/users/xhang/Projects/zcn/tmin/04xgb/02exp/xgb001/interim/tmin_005_clim_monthly.nc"
DEFAULT_OUT_PATH = "/public/home/ggao001/users/xhang/Projects/zcn/tmin/04xgb/02exp/xgb001/interim/tmin_001_clim_xgb.nc"
...
def to_numpy_2d_from_da(da: xr.DataArray, lat_name: str, lon_name: str) -> np.ndarray:
    ...
    if da.ndim > 2:
        ...
        for dim_name in dims:
            if dim_name not in (lat_name, lon_name):
                if da.sizes[dim_name] != 1:
                    raise ValueError(
                        f"Variable {da.name} has extra dim '{dim_name}' with size {da.sizes[dim_name]}; "
                        f"please manually select a 2D variable."
                    )
                squeeze_indexers[dim_name] = 0
...
def get_model_path(model_dir: str, month_int: int) -> str:
    return os.path.join(model_dir, f"xgb001_month_{month_int:02d}.joblib")
...
def build_feature_matrix_from_names(vars_proc: dict, feature_names):
    ...
    for name in feature_names:
        feature_arrays.append(vars_proc[name].astype(np.float32))
...
def main():
    parser = argparse.ArgumentParser(description="Predict 0.01-degree monthly climatology using trained XGB-001 models.")
    ...
    first_model_path = find_first_model_path(args.model_dir, "xgb001", train_months)
    ...
    ds_out = xr.Dataset(
        data_vars={
            "clim_tmin_xgb": (("month", "lat", "lon"), clim_out.astype(np.float32))
        },
        coords={
        ...
        attrs={
            "title": "XGB-001 predicted monthly climatology on 0.01-degree grid",
    ...
    ds_out["clim_tmin_xgb"].attrs = {
        "long_name": "XGB-001 predicted monthly climatology of Tmin on 0.01-degree grid",
        "units": "",
    }
    ...
    encoding = {"clim_tmin_xgb": {"zlib": True, "complevel": 4, "dtype": "float32"}}
