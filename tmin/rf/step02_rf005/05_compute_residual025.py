...
Aggregate 0.05-degree daily raw Tmin back to the 0.25-degree grid,
then compute daily residual on the 0.25-degree grid.

Inputs:
- tmin_005_raw.nc
- CN05.1_Tmin_1961_2025_daily_025x025_coastal.nc

Outputs:
- agg025_from_tmin005_raw.nc
...
DEFAULT_RAW005_PATH = "/public/home/ggao001/users/xhang/Projects/zcn/tmin/03random_f/02exp/rf005/interim/tmin_005_raw.nc"
DEFAULT_TMIN025_PATH = "/public/home/ggao001/users/xhang/Projects/CN_YANHAI_DOWN/01data/coastal_daily_025/CN05.1_Tmin_1961_2025_daily_025x025_coastal.nc"
DEFAULT_AGG_OUT = "/public/home/ggao001/users/xhang/Projects/zcn/tmin/03random_f/02exp/rf005/interim/agg025_from_tmin005_raw.nc"
DEFAULT_RES_OUT = "/public/home/ggao001/users/xhang/Projects/zcn/tmin/03random_f/02exp/rf005/interim/residual_025_daily.nc"
...
def load_raw005(path: str, var_name: str = None):
    log(f"Loading 0.05-degree raw Tmin: {path}")
    ...
    if var_name is not None:
        ...
        target_var = find_3d_var(
            ...
            preferred_names=["tmin_005_raw", "tmin", "Band1"]
        )
...
def load_target025(path: str, var_name: str = None):
    log(f"Loading 0.25-degree target Tmin: {path}")
    ds = xr.open_dataset(path)
    ...
    if var_name is not None:
        ...
        target_var = find_3d_var(
            ds, time_name, lat_name, lon_name,
            preferred_names=["tmin", "Tmin", "tasmax", "Band1"]
        )
...
def main():
    parser = argparse.ArgumentParser(description="Aggregate 0.05-degree raw Tmin to 0.25-degree and compute residual.")
    parser.add_argument("--raw005", default=DEFAULT_RAW005_PATH, help="Input 0.05-degree raw Tmin NetCDF")
    parser.add_argument("--target025", default=DEFAULT_TMIN025_PATH, help="Input original 0.25-degree Tmin NetCDF")
    ...
    parser.add_argument("--raw-var", default=None, help="Optional 0.05-degree raw Tmin variable name")
    parser.add_argument("--target-var", default=None, help="Optional 0.25-degree target Tmin variable name")
    ...
    agg_da = xr.DataArray(
        ...
        dims=("time", "lat", "lon"),
        name="agg_tmin_025_from_005_raw",
        attrs={
            "long_name": "0.05-degree raw Tmin aggregated back to 0.25-degree grid",
    ...
    res_da.name = "residual_tmin_025"
    res_da.attrs = {
        "long_name": "Daily residual on 0.25-degree grid",
        "units": da_tgt.attrs.get("units", ""),
        "definition": "original_tmin_025 - aggregated_tmin_from_005_raw",
    ...
    ds_agg = xr.Dataset(
        {"agg_tmin_025_from_005_raw": agg_da},
        attrs={
            "title": "Aggregated 0.25-degree Tmin from 0.05-degree raw Tmin",
    ...
    ds_res = xr.Dataset(
        {"residual_tmin_025": res_da},
        attrs={
            "title": "Daily 0.25-degree residual from raw 0.05-degree Tmin",
            "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "target025_file": args.target025,
            "target025_variable": tgt_var_name,
            "aggregated025_file": args.agg_out,
            "aggregated025_variable": "agg_tmin_025_from_005_raw",
    ...
    ds_agg.to_netcdf(
        args.agg_out,
        encoding={"agg_tmin_025_from_005_raw": {"zlib": True, "complevel": 4, "dtype": "float32"}}
    )

    log(f"Saving residual file: {args.res_out}")
    ds_res.to_netcdf(
        args.res_out,
        encoding={"residual_tmin_025": {"zlib": True, "complevel": 4, "dtype": "float32"}}
    )
