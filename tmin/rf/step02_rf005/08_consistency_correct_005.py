...
Steps:
1) Aggregate tmin_005_corr back to 0.25-degree grid
...
4) Save delta_025 and final tmin_005_final
...
DEFAULT_CORR005_PATH = "/public/home/ggao001/users/xhang/Projects/zcn/tmin/03random_f/02exp/rf005/interim/tmin_005_corr.nc"
...
DEFAULT_DELTA_OUT = "/public/home/ggao001/users/xhang/Projects/zcn/tmin/03random_f/02exp/rf005/interim/delta_025_consistency.nc"
DEFAULT_FINAL_OUT = "/public/home/ggao001/users/xhang/Projects/zcn/tmin/03random_f/02exp/rf005/outputs/tmin_005_final.nc"
...
def main():
    parser = argparse.ArgumentParser(description="Apply strict 0.25-degree consistency correction to 0.05-degree Tmin.")
    parser.add_argument("--corr005", default=DEFAULT_CORR005_PATH, help="Input residual-corrected 0.05-degree Tmin NetCDF")
    ...
    parser.add_argument("--final-out", default=DEFAULT_FINAL_OUT, help="Output final 0.05-degree Tmin NetCDF")
    ...
    ds_corr, da_corr, corr_time_name, corr_lat_name, corr_lon_name, corr_var_name = load_field(
        args.corr005,
        default_names=["tmin_005_corr", "tmin", "Band1"],
    ...
    ds_tgt, da_tgt, tgt_time_name, tgt_lat_name, tgt_lon_name, tgt_var_name = load_field(
        args.target025,
        default_names=["tmin", "Tmin", "tasmax", "Band1"],
    ...
    delta_da = xr.DataArray(
        ...
        attrs={
            ...
            "definition": "target_tmin_025 - aggregated_tmin_025_from_tmin_005_corr",
    ...
    final_da = xr.DataArray(
        ...
        name="tmin_005_final",
        attrs={
            "long_name": "Final daily Tmin on 0.05-degree grid after residual and consistency correction",
            "units": da_corr.attrs.get("units", da_tgt.attrs.get("units", "")),
            "definition": "tmin_005_corr + distributed_delta_025_consistency",
            "source_corr_file": args.corr005,
            "source_corr_variable": corr_var_name,
    ...
    ds_delta = xr.Dataset(
        ...
        attrs={
            "title": "0.25-degree consistency delta for 0.05-degree Tmin",
    ...
    ds_final = xr.Dataset(
        {"tmin_005_final": final_da},
        attrs={
            "title": "Final daily Tmin on 0.05-degree grid",
            "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "corr005_file": args.corr005,
            "corr005_variable": corr_var_name,
            "target025_file": args.target025,
    ...
    ds_final.to_netcdf(
        args.final_out,
        encoding={"tmin_005_final": {"zlib": True, "complevel": 4, "dtype": "float32"}}
    )
