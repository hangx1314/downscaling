...
Aggregate 0.01-degree daily raw Tmin back to the 0.05-degree grid,
...
DEFAULT_RAW001_PATH = "/public/home/ggao001/users/xhang/Projects/zcn/tmin/03random_f/02exp/rf001/interim/tmin_001_raw.nc"
DEFAULT_TMIN005_PATH = "/public/home/ggao001/users/xhang/Projects/zcn/tmin/03random_f/02exp/rf005/outputs/tmin_005_final.nc"
DEFAULT_AGG_OUT = "/public/home/ggao001/users/xhang/Projects/zcn/tmin/03random_f/02exp/rf001/interim/agg005_from_tmin001_raw.nc"
DEFAULT_RES_OUT = "/public/home/ggao001/users/xhang/Projects/zcn/tmin/03random_f/02exp/rf001/interim/residual_005_daily.nc"
...
def load_raw001(path: str, var_name: str = None):
    log(f"Loading 0.01-degree raw Tmin: {path}")
    ...
    if var_name is not None:
        ...
        target_var = find_3d_var(
            ...
            preferred_names=["tmin_001_raw", "tmin", "Band1"]
        )
...
def load_target005(path: str, var_name: str = None):
    log(f"Loading 0.05-degree target Tmin: {path}")
    ds = xr.open_dataset(path)
    ...
    if var_name is not None:
        ...
        target_var = find_3d_var(
            ds, time_name, lat_name, lon_name,
            preferred_names=["tmin_005_final", "tmin", "Band1"]
        )
...
def main():
    parser = argparse.ArgumentParser(description="Aggregate 0.01-degree raw Tmin to 0.05-degree and compute residual.")
    parser.add_argument("--raw001", default=DEFAULT_RAW001_PATH, help="Input 0.01-degree raw Tmin NetCDF")
    ...
    parser.add_argument("--raw-var", default=None, help="Optional 0.01-degree raw Tmin variable name")
    parser.add_argument("--target-var", default=None, help="Optional 0.05-degree target Tmin variable name")
    ...
    agg_da = xr.DataArray(
        ...
        dims=("time", "lat", "lon"),
        name="agg_tmin_005_from_001_raw",
        attrs={
            "long_name": "0.01-degree raw Tmin aggregated back to 0.05-degree grid",
    ...
    res_da.name = "residual_tmin_005"
    res_da.attrs = {
        "long_name": "Daily residual on 0.05-degree grid",
        "units": da_tgt.attrs.get("units", ""),
        "definition": "target_tmin_005 - aggregated_tmin_from_001_raw",
    ...
    ds_agg = xr.Dataset(
        {"agg_tmin_005_from_001_raw": agg_da},
        attrs={
            "title": "Aggregated 0.05-degree Tmin from 0.01-degree raw Tmin",
    ...
    ds_res = xr.Dataset(
        {"residual_tmin_005": res_da},
        attrs={
            "title": "Daily 0.05-degree residual from raw 0.01-degree Tmin",
            "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "target005_file": args.target005,
            "target005_variable": tgt_var_name,
            "aggregated005_file": args.agg_out,
            "aggregated005_variable": "agg_tmin_005_from_001_raw",
    ...
    ds_agg.to_netcdf(
        args.agg_out,
        encoding={"agg_tmin_005_from_001_raw": {"zlib": True, "complevel": 4, "dtype": "float32"}}
    )

    log(f"Saving residual file: {args.res_out}")
    ds_res.to_netcdf(
        args.res_out,
        encoding={"residual_tmin_005": {"zlib": True, "complevel": 4, "dtype": "float32"}}
    )
