...
"""
22_consistency_correct_001.py

Apply strict 0.05-degree consistency correction to the residual-corrected
0.01-degree daily Tmin field.

Steps:
1) Aggregate tmin_001_corr back to 0.05-degree grid
2) Compute delta_005 = target005 - aggregated005
3) Distribute delta_005 equally to all valid 0.01-degree subcells inside
   each 0.05-degree parent cell
4) Save delta_005 and final tmin_001_final
...
DEFAULT_CORR001_PATH = "/public/home/ggao001/users/xhang/Projects/zcn/tmin/03random_f/02exp/rf001/interim/tmin_001_corr.nc"
DEFAULT_TMIN005_PATH = "/public/home/ggao001/users/xhang/Projects/zcn/tmin/03random_f/02exp/rf005/outputs/tmin_005_final.nc"
DEFAULT_DELTA_OUT = "/public/home/ggao001/users/xhang/Projects/zcn/tmin/03random_f/02exp/rf001/interim/delta_005_consistency.nc"
DEFAULT_FINAL_OUT = "/public/home/ggao001/users/xhang/Projects/zcn/tmin/03random_f/02exp/rf001/outputs/tmin_001_final.nc"
...
def main():
    parser = argparse.ArgumentParser(description="Apply strict 0.05-degree consistency correction to 0.01-degree Tmin.")
    parser.add_argument("--corr001", default=DEFAULT_CORR001_PATH, help="Input residual-corrected 0.01-degree Tmin NetCDF")
    parser.add_argument("--target005", default=DEFAULT_TMIN005_PATH, help="Input original/final 0.05-degree Tmin NetCDF")
    parser.add_argument("--delta-out", default=DEFAULT_DELTA_OUT, help="Output 0.05-degree consistency delta NetCDF")
    parser.add_argument("--final-out", default=DEFAULT_FINAL_OUT, help="Output final 0.01-degree Tmin NetCDF")
    parser.add_argument("--corr-var", default=None, help="Optional corrected 0.01-degree variable name")
    parser.add_argument("--target-var", default=None, help="Optional target 0.05-degree variable name")
    parser.add_argument("--time-chunk", type=int, default=120, help="Time chunk size")
    ...
    ds_corr, da_corr, corr_time_name, corr_lat_name, corr_lon_name, corr_var_name = load_field(
        args.corr001,
        default_names=["tmin_001_corr", "tmin", "Band1"],
    ...
    ds_tgt, da_tgt, tgt_time_name, tgt_lat_name, tgt_lon_name, tgt_var_name = load_field(
        args.target005,
        default_names=["tmin_005_final", "tmin", "Band1"],
    ...
    mask2d = load_land_mask_2d(args.mask001, src_lat, src_lon)
    ...
    delta_da = xr.DataArray(
        ...
        dims=("time", "lat", "lon"),
        name="delta_005_consistency",
        attrs={
            "long_name": "Consistency delta on 0.05-degree grid",
            "units": da_tgt.attrs.get("units", ""),
            "definition": "target_tmin_005 - aggregated_tmin_005_from_tmin_001_corr",
            "source_corr_file": args.corr001,
    ...
    final_da = xr.DataArray(
        ...
        name="tmin_001_final",
        attrs={
            "long_name": "Final daily Tmin on 0.01-degree grid after residual and consistency correction",
            "units": da_corr.attrs.get("units", da_tgt.attrs.get("units", "")),
            "definition": "tmin_001_corr + distributed_delta_005_consistency",
            "source_corr_file": args.corr001,
            "source_corr_variable": corr_var_name,
            "target005_file": args.target005,
            "target005_variable": tgt_var_name,
            "distribution_method": "equal_addition_to_all_valid_001_subcells_within_each_005_parent",
            "grid_edge_mode": "clip",
        }
    )

    ds_delta = xr.Dataset(
        {"delta_005_consistency": delta_da},
        attrs={
            "title": "0.05-degree consistency delta for 0.01-degree Tmin",
            "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "corr001_file": args.corr001,
            "corr001_variable": corr_var_name,
            "target005_file": args.target005,
            "target005_variable": tgt_var_name,
            "grid_edge_mode": "clip",
        }
    ...
    ds_final = xr.Dataset(
        {"tmin_001_final": final_da},
        attrs={
            "title": "Final daily Tmin on 0.01-degree grid",
            "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "corr001_file": args.corr001,
            "corr001_variable": corr_var_name,
            "target005_file": args.target005,
    ...
    ds_delta.to_netcdf(
        args.delta_out,
        encoding={"delta_005_consistency": {"zlib": True, "complevel": 4, "dtype": "float32"}}
    )

    log(f"Saving final file: {args.final_out}")
    ds_final.to_netcdf(
        args.final_out,
        encoding={"tmin_001_final": {"zlib": True, "complevel": 4, "dtype": "float32"}}
    )
